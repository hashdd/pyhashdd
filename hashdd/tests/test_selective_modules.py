...
class TestFeatures(unittest.TestCase):
    ...
    def test_includes_removal(self):
        ...
        should be preset"""
        self.assertFalse('hashdd_known_level' in self.h.result)

    def test_includes(self):
        """When a list of features is provided, only those
        features should be present"""
        self.assertTrue('hashdd_file_name' in self.h.result)
    ...
    def test_overrides(self):
        ...
        self.assertEqual(self.h.result['hashdd_file_absolute_path'], 'test/')
        
        # Overrides not explicitedly defined in feature includes should not be in output
        self.assertFalse('hashdd_size' in self.h.result)

    def test_includes_algorithms(self):
        self.assertTrue('hashdd_md5w' in self.h.result)
        self.assertTrue('hashdd_crc16' in self.h.result)
